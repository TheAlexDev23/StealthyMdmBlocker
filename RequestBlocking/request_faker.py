...
def handle_response(response: str) -> str:
    ...
    elif command is CommandType.SecurityInfo:
        return send_security_info(command_uuid)
    else:
...
def send_security_info(command_uuid: str) -> str:
    return send_request(security_info(command_uuid), TARGET, "Security Info")
...
def security_info(command_uuid: str) -> str:
    return f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
